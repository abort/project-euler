...
def process_product(p, sequence):
        global max_product
        if (p > max_product):
                max_product = p
                print("new max product: {0} (sequence: {1})".format(p, sequence))
def get_max_product(): return max_product
...
def problem11(amount_of_adjacent_numbers = 4):
        M = get_grid()

        dimensions = len(M)
        assert(dimensions == len(M[0])) # block grid

        max_dimensions = dimensions - amount_of_adjacent_numbers + 1
        dimensions_range_vertical = range(0, max_dimensions)
        dimensions_range_horizontal = range(0, max_dimensions)
        for i in dimensions_range_vertical:
                for j in dimensions_range_horizontal:
                        # horizontal items
                        process_product(reduce(mul, M[i][j:amount_of_adjacent_numbers + j]), M[i][j:amount_of_adjacent_numbers + j])

        for i in dimensions_range_vertical:
                for j in range(0, dimensions):
                        # vertical
                        mult = []
                        for v in range(0, amount_of_adjacent_numbers):
                                mult.append(M[i:amount_of_adjacent_numbers + i][v][j])
                        process_product(reduce(mul, mult), mult)
        ...
        for i in dimensions_range_vertical:
                for j in range(0, dimensions):
                        sequence_forward = []
                        sequence_backward = []
                        exceeded_forward = False
                        exceeded_backward = False
                        for k in range(0, amount_of_adjacent_numbers):
                                y = j+k
                                x = i+k
                                if y >= dimensions or x >= dimensions:
                                        exceeded_forward = True
                                        break
                                sequence_forward.append(M[x][y])
                        for k in range(0, amount_of_adjacent_numbers):
                                y = j+k
                                x = i-k
                                if y >= dimensions or x < 0:
                                        exceeded_backward = True
                                        break
                                sequence_backward.append(M[x][y])
                        if (not exceeded_forward and len(sequence_forward) == amount_of_adjacent_numbers):
                                process_product(reduce(mul, sequence_forward), sequence_forward)

                        if (not exceeded_backward and len(sequence_backward) == amount_of_adjacent_numbers):
                                process_product(reduce(mul, sequence_backward), sequence_backward)

        return get_max_product()